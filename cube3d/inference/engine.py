...
class Engine:
    def __init__(
        ...
        Attributes:
            ...
            text_tokenizer (CLIPTokenizer): The tokenizer for the text model.
        ...
        self.device = device
        self.autocast_dtype = select_autocast_dtype(self.device)
        ...
        wte = cast(nn.Embedding, self.gpt_model.transformer.wte)
        wte.weight.data[: codebook.shape[0]] = codebook

        # We only use the text tower of CLIP; the vision tower's weights are
        # reported as "UNEXPECTED" by transformers v5's load report (a warning),
        # and the TP plan check warns about unsharded layers we never plan to
        # shard. Both are expected for this use case, so silence those loggers
        ...
        prev_levels = [lg.level for lg in silenced]
        for lg in silenced:
            lg.setLevel(logging.ERROR)
        try:
            self.text_model = self._load_clip_text_model(
                self.cfg.text_model_pretrained_model_name_or_path
            )
            self.text_tokenizer = self._load_clip_tokenizer(
                self.cfg.text_model_pretrained_model_name_or_path
            )
        finally:
            for lg, lvl in zip(silenced, prev_levels):
                lg.setLevel(lvl)
    ...
    def _load_clip_text_model(self, name_or_path: str) -> CLIPTextModelWithProjection:
        # Try the HF cache first so a cold network call (with its unauthenticated
        # rate-limit warning from the Hub) is avoided when the model is already
        # present locally. Fall back to a normal load when nothing is cached.
        try:
            model = CLIPTextModelWithProjection.from_pretrained(
                name_or_path, local_files_only=True
            )
        except (OSError, ValueError):
            model = CLIPTextModelWithProjection.from_pretrained(name_or_path)
        return cast(nn.Module, model).to(self.device).eval()

    def _load_clip_tokenizer(self, name_or_path: str) -> CLIPTokenizer:
        try:
            return CLIPTokenizer.from_pretrained(name_or_path, local_files_only=True)
        except (OSError, ValueError):
    ...
    def run_gpt(
        ...
        # Reused once-per-step position id buffer; avoids allocating a fresh
        # device tensor at every step of the decode loop.
        curr_pos_id = torch.zeros((1,), dtype=torch.long, device=embed.device)
        ...
        with torch.autocast(self.device.type, dtype=self.autocast_dtype):
            for i in tqdm(
                ...
                # When CFG is active, both halves of the batch share the same
                # token embedding; write each half without materializing a
                # doubled tensor.
                next_embed_flat = next_embed.squeeze(1)
                pos = i + input_seq_len
                if guidance_scale > 0.0:
                    half = batch_size // 2
                    embed_buffer[:half, pos, :].copy_(next_embed_flat)
                    embed_buffer[half:, pos, :].copy_(next_embed_flat)
                else:
                    embed_buffer[:, pos, :].copy_(next_embed_flat)
    ...
    def run_shape_decode(
        ...
        Args:
            ...
            chunk_size (int, optional): The chunk size for processing. Defaults to 250,000.
        Returns:
    ...
    def t2s(
        ...
        Args:
            ...
            chunk_size (int, optional): The chunk size for processing the shape decoding. Default is 250,000.
            top_p (float, optional): The cumulative probability threshold for nucleus sampling.
                If None, argmax selection is performed (deterministic generation). Otherwise, smallest set of tokens with cumulative probability ≥ top_p are kept (stochastic generation).
