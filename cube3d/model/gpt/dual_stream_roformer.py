...
class DualStreamRoformer(nn.Module):
    ...
    def encode_token(self, tokens):
        ...
        return cast(nn.Embedding, self.transformer.wte)(tokens)
    ...
    def init_kv_cache(
        ...
        kv_cache = [
            ...
            for _ in range(len(cast(nn.ModuleList, self.transformer.dual_blocks)))
        ]
        kv_cache += [
            Cache(
                key_states=torch.zeros(
            ...
            for _ in range(len(cast(nn.ModuleList, self.transformer.single_blocks)))
        ]
        return kv_cache
    ...
    def forward(
        ...
        layer_idx = 0
        for block in cast(nn.ModuleList, self.transformer.dual_blocks):
            h, c = block(
                h,
                c=c,
            ...
            layer_idx += 1
        for block in cast(nn.ModuleList, self.transformer.single_blocks):
            h = block(
                h,
        ...
        # Normalization
        h = cast(LayerNorm, self.transformer.ln_f)(h)
