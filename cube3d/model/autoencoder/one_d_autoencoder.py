...
class OneDAutoEncoder(nn.Module):
    ...
    def extract_geometry(
        ...
        """
        # warp-based marching cubes is CUDA-only. On MPS/CPU, skip warp
        # entirely and go straight to the skimage path with no warning spam.
        use_warp = use_warp and torch.cuda.is_available()
        ...
        for i in range(batch_size):
            try:
                ...
                if not warp_success:
                    # Only warn about falling back when a warp attempt was
                    # actually made and failed. When use_warp is False (e.g.
                    # MPS/CPU), skimage is the intended silent default.
                    if use_warp:
                        logging.warning(
